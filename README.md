...
| [database/README.md](database/README.md) | Scripts SQL (MariaDB) y creación de la base de datos |
...
2. **Base de datos (MariaDB local)**: crear/apuntar la base y ejecutar `database/01_CreateSchema.sql` según [database/README.md](database/README.md) — o simplemente `docker compose up --build` desde la raíz, que la crea automáticamente.
...
Variables del contenedor (local): usa la plantilla `docker/backend.local.env.mariadb.example` → copia a `docker/backend.local.env` (ignorado por git) y ejecuta:
